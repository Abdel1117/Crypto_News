...
export default function HeatMapTreemap({
  ...
  if (error) {
    return (
      <div
        className={`bg-card rounded-lg ${heightClassName} flex flex-col items-center justify-center gap-3 text-muted`}
      >
        <p>{error}</p>
        <button
          onClick={onRefresh}
  ...
  if (loading) {
    return (
      <div
        className={`bg-card rounded-lg ${heightClassName} animate-pulse flex items-center justify-center text-muted`}
      ></div>
    );
  }
  ...
  if (!data.children || data.children.length === 0) {
    return (
      <div
        className={`bg-card rounded-lg ${heightClassName} flex items-center justify-center text-muted`}
      >
  ...
  return (
    <div className={`bg-card rounded-lg ${heightClassName}`}>
      <ResponsiveTreeMap
        data={data}
        identity="name"
        value="marketCap"
